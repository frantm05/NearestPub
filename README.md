...
- **Deterministic mock world.** The map is a fixed global grid (~500 m cells); each cell seeds its own PRNG and spawns 0–2 venues with authentic Czech names, beers and 2026-realistic CZK prices. The same street corner always produces the same pubs across restarts. The world is geofenced to the Czech Republic — anywhere else the app shows an honest empty state instead of leaking South Bohemian pubs onto foreign maps. With location denied or unavailable, it falls back to České Budějovice's main square — never Prague — with a banner; the Jest suite pins this down.
